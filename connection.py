...
class Connection:
    ...
    def wait_for_socket_connection(self):
        ...
        print(f"Waited for {elapsed_time:.2f} seconds")
        print(f"Event was {'set' if result else 'not set'} within the timeout")
        if elapsed_time > 1:
            time.sleep(1000000)
        #   while True:
        #     if self.listener.connected_event.is_set():
        #         return
        #     elif self.listener.is_closed:
        ...
        #                 # self.set_idle()            
        #     ws.on_open = on_open
        #     print("run forever start")
        #     ws.run_forever()
            
        #     print("run forever emd")
    ...
    def _create_websocket(self):
        ...
        # for trace, _ in traceback.walk_stack(None):
        #     print(trace, self.websocket_url)          
    ...
    def listener_loop(self):
        ...
        while not self.cancel_event.is_set():
            ...
            def on_error(ws, error):
                self.set_idle()
                # print("Connection error", error, ws.url)
                # print(f"Error: {error}")
                pass

            def on_close(ws, x, w):
                self.is_closed = True
                # print("Connection closed", ws.url)
                self.set_idle()
                # ws.close()
                pass
            def on_open(ws):
                self.is_closed = False
                # print('Connection opened', ws.url)
                self.connected_event.set()
                self.set_idle()
            ...
            ws.run_forever(ping_timeout=900 # 15 minutes
)
            # See if works likely in Listner only the bug change is there
            if not self.cancel_event.is_set() and not self.connected_event.is_set():
                self.reconnect_ws()
                time.sleep(0.1)
