...
def make_element(driver:'Driver', current_tab:Tab, parent_tab:'BrowserTab',  internal_elem:CoreElement):
    if not internal_elem:
        return None
    if internal_elem._node.node_name == "IFRAME":
        return create_iframe_element(driver, current_tab, parent_tab, internal_elem)
    else:
        return Element(driver, current_tab, parent_tab,  internal_elem)
...
class Element:
    def __init__(self, driver:'Driver',  tab: Tab, parent_tab: 'BrowserTab', elem: CoreElement):
        self._driver = driver
    ...
    def _get_bounding_rect_with_iframe_offset(self):
        ...
        parent_rect = self.parent_tab._get_bounding_rect_with_iframe_offset()
    ...
    def _make_element(self, elem: CoreElement) -> "Element":
        return make_element(self._driver, self._tab, self.parent_tab, elem)
    ...
    def click(
        ...
        if selector:
            ...
            if self._driver.is_human_behavior_enabled:
    ...
    def click_at_point(self, x: int, y:int):
        rect = self.get_bounding_rect()
        if self._driver.is_human_behavior_enabled:
            with_human_behavior(self._driver, lambda: self._driver._cursor.click([x+rect.x, y+rect.y]))
        else:
            self._tab.click_at_point(x+rect.x, y+rect.y)

    def move_mouse_here(self):
        with_human_behavior(self._driver, lambda: self._driver._cursor.move_to(self))

    def move_mouse_to_point(self, x: int, y:int):
        rect = self.get_bounding_rect()
        with_human_behavior(self._driver, lambda: self._driver._cursor.move_mouse_to_point(x+rect.x, y+rect.y))
    ...
    def mouse_press(self, x: int, y:int):
        rect = self.get_bounding_rect()
        with_human_behavior(self._driver, lambda: self._driver._cursor.mouse_press(x+rect.x, y+rect.y))

    def mouse_release(self, x: int, y:int):
        rect = self.get_bounding_rect()
        with_human_behavior(self._driver, lambda: self._driver._cursor.mouse_release(x+rect.x, y+rect.y))
    ...
    def mouse_press_and_hold(self, x: int, y: int, release_condition: Optional[Callable[[], bool]] = None, release_condition_check_interval: float = 0.5, click_duration: Optional[float] = None):
        rect = self.get_bounding_rect()
        with_human_behavior(self._driver, lambda: self._driver._cursor.mouse_press_and_hold(x+rect.x, y+rect.y, release_condition=release_condition, release_condition_check_interval=release_condition_check_interval, click_duration=click_duration))
    ...
    def drag_and_drop_to(self, to_point: Union[tuple[int, int], 'Element']) -> None:
        ...
        Returns:
            None
        """
        with_human_behavior(self._driver, lambda: self._driver._cursor.drag_and_drop(self, to_point))
...
class BrowserTab:
    def __init__(self, config, _tab_value:Tab, parent_tab:'BrowserTab', _driver:'Driver',_browser:Browser):
        self.config = config
        self._tab_value = _tab_value
        self._browser:Browser = _browser
        self.parent_tab:Browser = parent_tab 
    ...
    def click_at_point(self, x: int, y:int):
        if self.is_human_behavior_enabled: 
            with_human_behavior(self, lambda: self._cursor.click([x,y]))
        else:
            self._tab.click_at_point(x, y)

    def move_mouse_to_point(self, x: int, y:int):
        with_human_behavior(self, lambda: self._cursor.move_mouse_to_point(x,y))
    ...
    def mouse_press(self, x: int, y:int):
        with_human_behavior(self, lambda: self._cursor.mouse_press(x,y))

    def mouse_release(self, x: int, y:int):
        with_human_behavior(self, lambda: self._cursor.mouse_release(x,y))

    def mouse_press_and_hold(self, x: int, y: int, release_condition: Optional[Callable[[], bool]] = None, release_condition_check_interval: float = 0.5, click_duration: Optional[float] = None):
        with_human_behavior(self, lambda: self._cursor.mouse_press_and_hold(x,y, release_condition=release_condition, release_condition_check_interval=release_condition_check_interval, click_duration=click_duration))
    ...
    def drag_and_drop(self, from_point: tuple[int, int], to_point: tuple[int, int] ) -> None:
            ...
            Returns:
                None
            """
            with_human_behavior(self, lambda: self._cursor.drag_and_drop(from_point, to_point))
            

    def get_iframe_by_link(
        self, link_regex: Optional[str] = None, wait: Optional[int] = Wait.SHORT
    ):
        return get_iframe_elem_by_link(self._driver,  self._tab, self.parent_tab,  link_regex, wait)
...
class IframeTab(BrowserTab):
    ...
    def __init__(self, iframe_elem:Element, config, _tab_value:Tab, parent_tab:'BrowserTab', driver:'Driver', _browser:Browser):
        self.iframe_elem = iframe_elem
        super().__init__(config, _tab_value, parent_tab, driver, _browser)
    ...
    def _get_bounding_rect_with_iframe_offset(self):
        rect = self.parent_tab._get_bounding_rect_with_iframe_offset()
...
class IframeElement(BrowserTab):
    def __init__(self, elem: Element, doc_elem: Element, config, _tab_value:Tab, parent_tab:'BrowserTab',driver:'Driver',  _browser:Browser):
        self.elem  = elem
...
def get_iframe_element_or_tab(iframe_tab:Tab, driver:'Driver', current_tab:Tab, parent_tab:BrowserTab, internal_elem:CoreElement):
    if iframe_tab:
        wait_for_iframe_tab_load(driver, iframe_tab)
        iframe_elem = Element(driver, current_tab,  parent_tab,internal_elem)
        return IframeTab(iframe_elem, driver.config, iframe_tab, parent_tab, driver, driver._browser)
    internal_elem.tree = internal_elem.content_document
    elem = Element(driver,  current_tab, parent_tab,internal_elem)
    doc_elem = Element(driver,   current_tab, parent_tab, CoreElement(internal_elem.content_document, current_tab, internal_elem.tree))
    return IframeElement(elem, doc_elem, driver.config, current_tab,  parent_tab, driver, driver._browser)

def create_iframe_element(driver:'Driver', current_tab:Tab, parent_tab:BrowserTab, internal_elem:CoreElement):
    iframe_tab = get_iframe_tab(driver, internal_elem)
    return get_iframe_element_or_tab(iframe_tab, driver, current_tab, parent_tab, internal_elem)


def get_iframe_elem_by_link(driver:'BrowserTab', current_tab:Tab, parent_tab:BrowserTab, link, timeout):
    iframe_tab = get_iframe_tab_by_link(driver, link, timeout)
    el = select_and_find_iframe_el(driver, link)
    if iframe_tab:
        return get_iframe_element_or_tab(iframe_tab, driver, current_tab, parent_tab, el._elem)
...
class Driver(BrowserTab):
    ...
    def disable_human_behavior(self):
        if self._cursor:
    ...
    @property
    def is_human_behavior_enabled(self):
